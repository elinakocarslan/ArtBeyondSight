...
export async function testDatabaseIntegration() {
    ...
    try {
        ...
        const savedAnalysis = await ArtBeyondSightAPI.saveMuseumAnalysis(testData);
        ...
        if (savedAnalysis.id) {
            const retrieved = await ArtBeyondSightAPI.getAnalysisById(savedAnalysis.id);
        ...
        if (savedAnalysis.id) {
            ...
            await ArtBeyondSightAPI.deleteAnalysis(savedAnalysis.id);
